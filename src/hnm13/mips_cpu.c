...
mips_error mem_base(mips_cpu_h state, itype operands, bool load, int length, uint8_t* word, int offset, int align)
{
	...
	uint32_t new_addr, new_len, data_offset;
	uint64_t data;
	uint8_t* ptr;
...
mips_error copz(mips_cpu_h state, uint32_t instruction)
{
	...
	mips_error error = cop(state, instruction);
	if(!error)
		advance_pc(state);
	return error;
}
...
mips_error lwcz(mips_cpu_h state, uint32_t instruction)
{
	...
	mips_error error = mem_base(state, operands, true, 4, (uint8_t*)&data, 0, 4);
	if(error)
		return error;
...
mips_error sh(mips_cpu_h state, uint32_t instruction)
{
	...
	reverse_half(&word);
	mips_error error = mem_base(state, operands, false, 2, (uint8_t*)&word, 0, 2);
	if(error)
		return error;
	advance_pc(state);
	return mips_Success;
...
mips_error sw(mips_cpu_h state, uint32_t instruction)
{
	...
	reverse_word(&word);
	mips_error error = mem_base(state, operands, false, 4, (uint8_t*)&word, 0, 4);
	if(error)
		return error;
	advance_pc(state);
	return mips_Success;
...
mips_error swl(mips_cpu_h state, uint32_t instruction)
{
	...
	reverse_half(&word);
	mips_error error = mem_base(state, operands, false, 2, (uint8_t*)&word, 0, 1);
	if(error)
		return error;
	advance_pc(state);
	return mips_Success;
...
mips_error swr(mips_cpu_h state, uint32_t instruction)
{
	...
	uint16_t word = state->reg[operands.d];
	reverse_half(&word);
	mips_error error = mem_base(state, operands, false, 2, (uint8_t*)&word, -1, 1);
	if(error)
		return error;
	advance_pc(state);
	return mips_Success;
