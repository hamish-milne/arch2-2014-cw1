...
{
	if(state->debug > 2)
		debug(state, temp_buf, sprintf(temp_buf, "Delayed jump to 0x%x\n", value));
...
{
	if(state->debug > 2)
		debug(state, temp_buf, sprintf(temp_buf, "Actual jump to 0x%x\n", state->jump_addr));
...
int main()
{
	...
	mips_load_file(state->mem, "fragments/f_addu-mips.bin");
	mips_cpu_step(state);
	mips_cpu_step(state);
	mips_cpu_step(state);
	mips_cpu_step(state);
